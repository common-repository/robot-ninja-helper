...
class RN_API {
	...
	public static function init() {
		...
		add_filter( 'woocommerce_rest_is_request_to_rest_api', __CLASS__ . '::authenticate_with_woocommerce' );
	}
	...
	public static function add_robot_ninja_data( $response, $field_name, $request ) {
		...
		$info->plugins            = apply_filters( 'rn_helper_plugin_support_settings', array() );
	...
	/**
	 * Allow API requests outside of the `wc/v2` namespace to be authenticated by WooCommerce Basic Auth/Query String
	 * This function only passes if the request is a Robot Ninja request.
	 *
	 * @since 1.9
	 * @param bool $is_wc_request
	 */
	public static function authenticate_with_woocommerce( $is_wc_request ) {
		if ( ! $is_wc_request && ! empty( $_SERVER['REQUEST_URI'] ) && robot_ninja_helper()->is_robot_ninja_ip() ) {
			$rest_prefix = trailingslashit( rest_get_url_prefix() );

			// Authenticate the /wp/v2/comments/ID requests coming from Robot Ninja using the WooCommerce methods (Basic Auth/Query String) using consumer key/secret
			if ( false !== strpos( $_SERVER['REQUEST_URI'], $rest_prefix . 'wp/v2/comments/' ) ) {
				$is_wc_request = true;
			}
		}
