...
class Robot_Ninja_Helper {
	...
	public function load_includes() {
		...
		require_once( dirname( __FILE__ ) . '/includes/class-rn-product-manager.php' );

		// include theme support files
		require_once( dirname( __FILE__ ) . '/includes/themes/class-rn-avada.php' );

		// include plugin support files
		require_once( dirname( __FILE__ ) . '/includes/plugins/class-rn-checkout-for-woocommerce.php' );
	}
	...
	/**
	 * Determine if the current request is from one of our own IP addresses and return true.
	 * Also returns true if IP address is local.
	 *
	 * @since 1.9
	 * @return bool
	 */
	public function is_robot_ninja_ip() {
		$ip_address = '';
		...
		if ( ! empty( $ip_address ) && in_array( $ip_address, array( '127.0.0.1', '54.70.117.80', '34.208.196.99' ) ) ) {
			return true;
		} else {
			return false;
		}
